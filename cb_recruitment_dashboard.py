...
league_avg_percentiles = filtered_df[[m + " Percentile" for m in pizza_metrics if m + " Percentile" in filtered_df.columns]].mean().values
...
# --- Dashboard ---
st.title("⚽ Centre-Back Recruitment Dashboard")
...
def plot_pizza(player, data, league_avg, metrics_list):
    ...
    # League Average Layer - Bright Yellow, with black edges on slices and label boxes
    fig, ax = pizza.make_pizza(
        league_avg_raw,
        figsize=(6, 6),
        color_blank_space="same",
        kwargs_slices=dict(
            facecolor="#FFFF00",   # Bright yellow
            edgecolor="black",     # Black edge now
            linewidth=1.5,
            alpha=1,            # Less transparent
            zorder=5
        ),
        kwargs_params=dict(
            color="black",
        ...
        kwargs_values=dict(
            ...
            fontsize=8,
            fontweight="bold",
            zorder=10,  # Bring labels forward
            bbox=dict(edgecolor="black", facecolor="#FFFF00", boxstyle="round,pad=0.15")  # Black edge on label boxes
        )
    )

    # Overwrite league average labels with percentiles, force redraw
    ...
    # Player Layer - Blue, Transparent
    pizza.make_pizza(
        player_percentiles_raw,
        ax=ax,
        color_blank_space="same",
        kwargs_slices=dict(
            facecolor="#1a78cf",   # Strong blue
            edgecolor="black",
            linewidth=2,
            alpha=0.8,             # More transparent to see league layer beneath
            zorder=5
        ),
        kwargs_params=dict(
            color="black",
        ...
        kwargs_values=dict(
            ...
            fontsize=8,
            fontweight="bold",
            zorder=20,  # Ensure player labels are topmost
            bbox=dict(edgecolor="#000000", facecolor="#1a78cf", boxstyle="round,pad=0.2", alpha=0.7)
        )
    )

    # Overwrite player labels with percentiles, bring to front
