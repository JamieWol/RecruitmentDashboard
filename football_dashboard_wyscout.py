...
def plot_pizza(player,data,league_avg,metrics_list):
    ...
    player_percentiles_str = [f"{int(p)}%" for p in player_percentiles]
    league_percentiles_str = [f"{int(p)}%" for p in league_percentiles]
    ...
    fig,ax = pizza.make_pizza(
        ...
        kwargs_slices=dict(facecolor="#FFFF00",edgecolor="black",linewidth=1.5),
...
if uploaded_file:
    ...
    for m in metrics:
        ...
        if m in df.columns:
            ...
            df[m+" Percentile"]=(df[m].rank(pct=True)*100).round(0).astype(int)
    ...
    # Overall Score
    df["Overall Score"]=df[percentile_columns].mean(axis=1).round(0).astype(int)
    ...
    st.subheader("Radar Chart: Player vs Player")
    ...
    st.download_button(
        ...
        data=csv,
        file_name="filtered_wyscout.csv",
        mime="text/csv"
    )
