...
export default function ScoutingReportsPage() {
  ...
  return <div className="sr-page">
    ...
    <div className="sr-grid">{visible.map(a => <button className="sr-card" key={a.id} onClick={() => open(a)}><div className="sr-card-top"><strong>{a.player}</strong><span className={`sr-status ${a.status === "Complete" ? "done" : ""}`}>{a.status}</span></div><p>{a.club || "Club not added"} · {a.position || "Position not added"}</p><small>{a.game || "Game not added"} {a.date && `· ${a.date}`} · {a.viewing}</small><small>Assigned to: {a.scout || "Unassigned"}</small></button>)}</div>
