        ...
        {!rows.length ? <div style={{ padding: "90px 20px", textAlign: "center", color: "#d7e8f8", fontSize: 20 }}>Upload a league team spreadsheet to build the dashboard.</div> : (
          <>
            ...
            <section style={{ marginTop: 28, display: "grid", gridTemplateColumns: "minmax(260px, .8fr) minmax(420px, 1.8fr)", gap: 24 }}>
              ...
              <div style={{ background: "#fff", color: "#123", borderRadius: 14, padding: "24px 28px", border: "2px solid #2080bd" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap", marginBottom: 22 }}><h2 style={{ color: "#1680bd", margin: 0 }}>Metric Percentiles</h2><div style={{ display: "flex", gap: 18, flexWrap: "wrap", fontSize: 12, fontWeight: 700 }}><span><i style={{ display: "inline-block", width: 13, height: 13, borderRadius: 4, background: "#15c77a", marginRight: 6, verticalAlign: "-2px" }} />Top 25%</span><span><i style={{ display: "inline-block", width: 13, height: 13, borderRadius: 4, background: "#ff9f1a", marginRight: 6, verticalAlign: "-2px" }} />50%</span><span><i style={{ display: "inline-block", width: 13, height: 13, borderRadius: 4, background: "#ff4740", marginRight: 6, verticalAlign: "-2px" }} />Below 25%</span><span><i style={{ display: "inline-block", width: 13, height: 13, borderRadius: 4, background: "#ffd21c", marginRight: 6, verticalAlign: "-2px" }} />League Average</span></div></div>{chartData.map((item) => <div key={item.metric} style={{ display: "grid", gridTemplateColumns: "220px minmax(0,1fr)", alignItems: "center", gap: 18, marginBottom: 16 }}><div><strong style={{ display: "block", fontSize: 15, color: "#111" }}>{item.metric}</strong><small style={{ color: "#667" }}>{item.teamValue}</small></div><div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 800, color: "#111", marginBottom: 4 }}><span>Team percentile</span><span>{item.percentile}%</span></div><div style={{ height: 13, borderRadius: 8, background: "#e2e4e7", overflow: "hidden" }}><div style={{ width: `${item.percentile}%`, height: "100%", borderRadius: 8, background: scoreColour(item.percentile) }} /></div><div style={{ height: 13, marginTop: 5, borderRadius: 8, background: "#f0f1f2", overflow: "hidden" }}><div style={{ width: `${item.leagueAveragePct}%`, height: "100%", borderRadius: 8, background: "#ffd21c" }} /><span style={{ position: "relative", display: "block", marginTop: -13, textAlign: "center", fontSize: 10, fontWeight: 800, color: "#111" }}>{item.leagueAveragePct}</span></div></div></div>)}</div>
            </section>
