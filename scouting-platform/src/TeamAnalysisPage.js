...
const metricGroup = (metric) => {
  ...
  if (/tackle|intercept|clearance|pressure|regain|defensive|duel|block|conceded|faced|against|ppda|aerial|defend|offside/.test(label)) return "Out Of Possession";
  return "In Possession";
};
const styleGroup = (metric) => {
  const label = String(metric || "").toLowerCase();
  if (/set.?piece|corner|free.?kick|dead.?ball|throw.?in|dfk/.test(label)) return "Set-Pieces";
  if (/tackle|intercept|clearance|pressure|regain|defensive|duel|block|conceded|faced|against|ppda|aerial|defend|offside/.test(label)) return "Defensive Work";
...
export default function TeamAnalysisPage() {
  ...
  const comparisonData = activeMetrics.map((metric) => {
    ...
    return { metric, first, second, firstPct: Math.round((first / scale) * 100), secondPct: Math.round((second / scale) * 100), difference, differencePct: Math.min(100, Math.round((Math.abs(difference) / scale) * 100)) };
  });
