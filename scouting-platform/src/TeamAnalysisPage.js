      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        {!rows.length ? <div style={{ padding: "90px 20px", textAlign: "center", color: "#d7e8f8", fontSize: 20 }}>Upload a league team spreadsheet to build the dashboard.</div> : (
          <>
            ...
            <ChartBoundary><section style={{ order: 3, marginTop: 24, background: "#fff", color: "#123", border: "2px solid #2080bd", borderRadius: 14, padding: "20px 28px" }}><h2 style={{ color: "#1680bd", margin: "0 0 4px", textAlign: "center" }}><span>Team</span><span style={{ marginLeft: "0.3em" }}>Style</span></h2><p style={{ color: "#667", marginTop: 0, textAlign: "center" }}>Profile based on the uploaded metric percentiles.</p><ResponsiveContainer width="100%" height={300}><RadarChart data={styleData} cx="50%" cy="50%" outerRadius="68%"><PolarGrid /><PolarAngleAxis dataKey="style" tick={{ fontSize: 12, fill: "#123" }} /><Radar name="Team style" dataKey="score" stroke="#1680bd" fill="#1680bd" fillOpacity={0.45} /></RadarChart></ResponsiveContainer></section></ChartBoundary>
            </div>
          </>
        )}
      </div>
