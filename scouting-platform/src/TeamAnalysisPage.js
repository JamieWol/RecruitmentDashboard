      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        {!rows.length ? <div style={{ padding: "90px 20px", textAlign: "center", color: "#d7e8f8", fontSize: 20 }}>Upload a league team spreadsheet to build the dashboard.</div> : (
          <>
            ...
            <section style={{ marginTop: 28, padding: "22px 28px", borderRadius: 14, background: "linear-gradient(110deg,#0b3c73,#155b91)", border: "2px solid #78b4d8", textAlign: "center" }}><h2 style={{ margin: "0 0 7px", color: "#fff", fontSize: 32 }}>{selectedTeamName} Team Analysis</h2><div style={{ color: "#d2e5fa", fontSize: 14 }}>{gamesKey ? `Games Played: ${normalise(selected?.[gamesKey]) || "-"}` : ""} · {metrics.length} metrics available</div></section>
            ...
            <ChartBoundary><section style={{ order: 3, marginTop: 24, background: "#173f70", color: "#fff", border: "2px solid #6a96bd", borderRadius: 14, padding: "20px 28px" }}><h2 style={{ color: "#62dcff", margin: "0 0 4px", textAlign: "center" }}>Team Visual Comparison</h2><p style={{ color: "#d2e5fa", marginTop: 0, textAlign: "center" }}>Compare the selected teams across the uploaded metrics.</p><div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 24, alignItems: "stretch" }}><section style={{ background: "#123761", borderRadius: 10, padding: 16 }}><h3 style={{ margin: "0 0 4px", color: "#fff", fontSize: 18, textAlign: "center" }}>Team Style</h3><p style={{ margin: "0 0 8px", color: "#d2e5fa", fontSize: 12, textAlign: "center" }}>Compared with league average</p><ResponsiveContainer width="100%" height={300}><RadarChart data={styleData} cx="50%" cy="50%" outerRadius="68%"><PolarGrid stroke="#6a96bd" /><PolarAngleAxis dataKey="style" tick={{ fontSize: 12, fill: "#d2e5fa" }} /><Radar name="League Average" dataKey="leagueAverage" stroke="#ffd21c" fill="#ffd21c" fillOpacity={0.28} /><Radar name={normalise(selected?.[teamKey]) || "Team"} dataKey="score" stroke="#62dcff" fill="#62dcff" fillOpacity={0.5} /></RadarChart></ResponsiveContainer></section><section style={{ background: "#123761", borderRadius: 10, padding: 16 }}><h3 style={{ margin: "0 0 4px", color: "#fff", fontSize: 18, textAlign: "center" }}>Team Difference</h3><p style={{ margin: "0 0 14px", color: "#d2e5fa", fontSize: 12, textAlign: "center" }}>{selectedTeamName} vs {comparisonTeamName || "second team"}</p><div style={{ display: "flex", justifyContent: "space-around", color: "#fff", fontSize: 11, marginBottom: 10 }}><span>● {selectedTeamName}</span><span style={{ color: "#ffd21c" }}>● {comparisonTeamName || "Comparison"}</span></div>{comparisonData.slice(0, 10).map((item) => <div key={item.metric} style={{ marginBottom: 10 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, fontSize: 11, color: "#fff" }}><span>{item.metric}</span><strong>{item.difference > 0 ? "+" : ""}{item.difference}</strong></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginTop: 3 }}><div style={{ height: 10, borderRadius: 6, background: "#365b80", overflow: "hidden" }}><div style={{ width: `${item.firstPct}%`, height: "100%", background: "#62dcff", borderRadius: 6 }} /></div><div style={{ height: 10, borderRadius: 6, background: "#284d73", overflow: "hidden" }}><div style={{ width: `${item.secondPct}%`, height: "100%", background: "#ffd21c", borderRadius: 6 }} /></div></div></div>)}</section></div></section></ChartBoundary>
            </div>
          </>
        )}
      </div>
