...
export async function syncPublishedReports(user, state, club) {
  ...
  const reports = (state?.assignments || []).filter((x) => ["Published", "Complete"].includes(x.status) && x.report).map((x) => ({
    ...
    player: x.player, club, author_id: user.id, report: x.report, status: "Published", completed_at: x.completedAt || x.date || new Date().toISOString().slice(0, 10), scout: x.scout || "", fixture_summary: (x.games || []).length > 1 ? "Multiple" : (x.games?.[0] ? `${x.games[0].date || ""} · ${x.games[0].name || x.games[0]}` : x.game || ""),
  }));
  if (reports.length) {
