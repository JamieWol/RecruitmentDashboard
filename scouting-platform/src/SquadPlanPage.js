...
const photoCandidates = (name) => { const raw = String(name || "").trim(); const short = raw.split(/\s+/).length > 2 ? `${raw.split(/\s+/)[0]} ${raw.split(/\s+/).at(-1)}` : raw; const nameVariants = [raw, short, raw.replace(/macaulay/gi, "macauley"), short.replace(/macaulay/gi, "macauley"), raw.replace(/macauley/gi, "macaulay"), short.replace(/macauley/gi, "macaulay")]; const bases = [...new Set(nameVariants.map((x) => x.replace(/[^\p{L}\p{N}]+/gu, "_").replace(/^_+|_+$/g, "")))]; return [...new Set(bases.flatMap((x) => [x, x.toLowerCase(), x.toUpperCase(), `_${x}`, `_${x.toLowerCase()}`, `__${x}`, x.normalize("NFD").replace(/[̀-ͯ]/g, "")]).map((x) => `${photoBase}${x}.png`))]; };
