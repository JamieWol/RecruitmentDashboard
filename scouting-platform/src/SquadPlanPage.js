...
const formationRows = {
  ...
  "3-5-2": [["ST", "ST"], ["LM", "CM", "CM", "RM"], ["LCB", "CB", "RCB"], ["GK"]],
  "3-4-3": [["LW", "CF", "RW"], ["LM", "LCM", "RCM", "RM"], ["LCB", "CB", "RCB"], ["GK"]],
  "4-1-4-1": [["ST"], ["LM", "CM", "CM", "RM"], ["DM"], ["LB", "LCB", "RCB", "RB"], ["GK"]],
};
const positionAliases = { goalkeeper: "GK", gk: "GK", defender: "CB", "centre-back": "CB", "center-back": "CB", fullback: "RB", "right-back": "RB", "left-back": "LB", midfielder: "CM", "central midfielder": "CM", winger: "RW", forward: "ST", striker: "ST" };
...
export default function SquadPlanPage() {
  ...
  const clubPlayers = useMemo(() => players.filter((p) => normalise(playerClub(p)) === normalise(club)), [players, club]);
  ...
  const zone = (position, index) => {
    ...
    return <div className="sr-zone" key={slot} data-count={`${listed.length} player${listed.length === 1 ? "" : "s"}`}><div className="sr-zone-head"><strong>{position}</strong><span>{listed.length ? listed.length : ""}</span></div><div className="sr-zone-list">{listed.map((p) => <div className="sr-pitch-player-card" key={String(p.id)}><button type="button"><img className="sr-shortlist-player-photo" src={photoFor(p.player)} alt="" onError={(e) => { e.currentTarget.style.display = "none"; }} /><span><strong>{p.player}</strong><small>{p.club}</small></span></button><button className="sr-slot-remove" type="button" onClick={() => remove(p.id)}>×</button></div>)}</div></div>;
  };

  return <main className="sr-page sr-shortlist-view sr-squad-plan"><button className="sr-back" onClick={() => window.history.back()}>‹ Back</button><section className="sr-dashboard-head"><div><div className="sr-kicker">SQUAD PLAN</div><h1>Plan Your Squad</h1><p>Choose a club to automatically load its players into a formation plan.</p></div><div className="sr-shortlist-head-actions"><label className="sr-field"><span>Club</span><select className="sr-formation-select" value={club} onChange={(e) => { setClub(e.target.value); localStorage.setItem("squadPlanClub", e.target.value); }}><option value="">Select club</option>{clubs.map((x) => <option key={x}>{x}</option>)}</select></label><label className="sr-field"><span>Formation</span><select className="sr-formation-select" value={formation} onChange={(e) => changeFormation(e.target.value)}>{formations.map((x) => <option key={x}>{x}</option>)}</select></label></div></section>{loading && <div className="sr-empty">Loading players…</div>}{!loading && club && !clubPlayers.length && <div className="sr-empty">No players found for this club.</div>}{club && clubPlayers.length > 0 && <div className="sr-tag-legend"><span>{clubPlayers.length} players loaded</span><span>{currentPlayers.length} in squad plan</span></div>}<div className="sr-real-pitch"><div className="sr-goal-box top" />{rows.map((row, i) => <div className="sr-pitch-row" key={i}>{row.map((pos, j) => zone(pos, j))}</div>)}<div className="sr-centre-circle" /><div className="sr-halfway-line" /><div className="sr-goal-box bottom" /></div></main>;
}
